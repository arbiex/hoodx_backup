  ...
  try {
    ...
    if (!isInternalCall && !SimpleSessionAffinity.shouldServeUser(request)) {
      ...
      if (sessionInstanceId) {
        console.log(`🔄 [SESSION-AFFINITY-BMGBR2-BET] Redirecionando para instância: ${sessionInstanceId}`);
        return new Response(
    ...
    if (!userId) {
      return createBMGBR2BetSessionResponse(NextResponse.json({
        success: false,
...
// 🔗 HELPER: Wrapper para adicionar cookie de afinidade de sessão no bet BMGBR2
function createBMGBR2BetSessionResponse(response: NextResponse): NextResponse {
