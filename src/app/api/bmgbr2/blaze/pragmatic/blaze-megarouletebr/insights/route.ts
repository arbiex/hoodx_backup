  ...
  try {
    ...
    if (!isInternalCall && !SimpleSessionAffinity.shouldServeUser(request)) {
      ...
      if (sessionInstanceId) {
        console.log(`🔄 [SESSION-AFFINITY-BMGBR2-INSIGHTS] Redirecionando para instância: ${sessionInstanceId}`);
        return new Response(
    ...
    if (!user_id) {
      return createBMGBR2InsightsSessionResponse(NextResponse.json(
    ...
    switch (action) {
      ...
      case 'get-full':
        ...
        return createBMGBR2InsightsSessionResponse(NextResponse.json(result));
...
// 🔗 HELPER: Wrapper para adicionar cookie de afinidade de sessão no insights BMGBR2
function createBMGBR2InsightsSessionResponse(response: NextResponse): NextResponse {
