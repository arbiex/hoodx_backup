    ...
    if (!isInternalCall && !SimpleSessionAffinity.shouldServeUser(request)) {
      ...
      if (sessionInstanceId) {
        ...
        if (loopCheck.hasLoop) {
          console.error(`❌ [SESSION-AFFINITY-INSIGHTS] LOOP detectado! Forçando aceitação.`);
          // Continuar processamento na instância atual
        } else {
          console.log(`🔄 [SESSION-AFFINITY-INSIGHTS] Redirecionando para instância: ${sessionInstanceId} (tentativa ${loopCheck.redirectCount + 1})`);
