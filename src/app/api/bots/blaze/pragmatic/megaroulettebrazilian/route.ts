...
interface MegaRouletteConfig {
  ...
  language?: string;
  currency?: string;
  action?: 'bet-connect' | 'bet-place' | 'bet-state' | 'get-websocket-logs' | 'monitor-patterns' | 'get-selected-pattern' | 'clear-selected-pattern' | 'start-auto-betting' | 'stop-auto-betting' | 'get-auto-betting-status' | 'configure-auto-betting';
  jsessionId?: string;
  gameConfig?: any;
    ...
    ws.on('message', (data: any) => {
      ...
      if (message.includes('<betsopen')) {
        ...
        if (gameMatch) {
            ...
            if (betting?.active && !betting.waitingForResult && selectedPattern?.martingale_pattern) {
            ...
            } else if (betting?.active && !selectedPattern?.martingale_pattern) {
              addWebSocketLog(userId, `⚠️ Apostas automáticas ativas mas padrão inválido - aguardando padrão válido`, 'error');
            }
          }
        }
...
function startPatternMonitoringInternal(userId: string) {
  ...
  patternMonitoring[userId] = {
    ...
    waitingForNewResult: true, // Aguardar novo resultado antes de selecionar padrão
...
async function startAutoBetting(userId: string) {
  try {
    ...
    if (!selectedPattern) {
      console.log('❌ [AUTO-BET] Nenhum padrão selecionado');
      return NextResponse.json({
        success: false,
        error: 'Nenhum padrão selecionado. Selecione um padrão primeiro.'
      }, { status: 400 });
    }
    ...
    // Validar martingale pattern
    const martingalePattern = selectedPattern.martingale_pattern;
    if (!martingalePattern || martingalePattern.length === 0) {
      return NextResponse.json({
        success: false,
    ...
    // Verificar se todas as letras do padrão são válidas
    ...
    autoBetting[userId] = {
      active: true,
      currentBetIndex: 0,
      totalBets: Math.min(martingalePattern.length, sequences.length), // Usar o menor entre padrão e sequências
      pattern: martingalePattern,
      betAmount: sequences[0]?.bet_amount || 0.5, // Primeira aposta da sequência
      wins: 0,
      losses: 0,
      startedAt: Date.now(),
    ...
    addWebSocketLog(userId, `🎯 Padrão: ${martingalePattern} (${autoBetting[userId].totalBets} apostas)`, 'info');
    ...
    return NextResponse.json({
      success: true,
      data: {
        message: 'Apostas automáticas iniciadas com sucesso',
        pattern: martingalePattern,
        totalBets: martingalePattern.length,
        betAmount: 0.5,
        patternInfo: {
...
function processAutoBetResult(userId: string, resultNumber: number, resultColor: string) {
  try {
    ...
    if (isWin) {
      ...
      const totalInvested = betting.currentBetIndex * betting.betAmount;
      const profit = betting.betAmount; // Lucro da vitória (valor da aposta vencedora)
      ...
      addWebSocketLog(userId, `💰 Estatísticas finais: ${betting.wins} vitória(s), ${betting.losses} derrota(s). Investido: R$ ${totalInvested}, Lucro: R$ ${profit}`, 'success');
      ...
      setTimeout(() => {
        try {
          ...
          startPatternMonitoringInternal(userId);
          addWebSocketLog(userId, `🎯 Loop automático ativado - aguardando próximo padrão`, 'success');
        } catch (error) {
    ...
    if (betting.currentBetIndex >= betting.totalBets) {
      ...
      const totalInvested = betting.totalBets * betting.betAmount;
      ...
      addWebSocketLog(userId, `🏁 SEQUÊNCIA COMPLETADA SEM VITÓRIAS! ${betting.wins}/${betting.totalBets} vitórias (${winRate}%). Total investido: R$ ${totalInvested}`, 'error');
