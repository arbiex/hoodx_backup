...
async function sendBetViaPragmaticWebSocket(config: {
  ...
  <lpbet gm="roulette_desktop" gId="${currentGameId}" uId="${config.pragmaticUserId}" ck="${timestamp}">
    <bet amt="${config.amount}" bc="${config.betCode}" ck="${timestamp}" />
          ...
          // Tentar envio robusto
          try {
            ws.send(betXml);
            betSent = true;
            console.log('✅ Comando de aposta enviado via método direto');
          } catch (sendError: any) {
            console.log('⚠️ Método direto falhou, tentando Buffer:', sendError.message);
            try {
              const buffer = Buffer.from(betXml, 'utf8');
              ws.send(buffer);
              betSent = true;
              console.log('✅ Comando de aposta enviado via Buffer');
            } catch (bufferError: any) {
              console.error('❌ Todos os métodos de envio falharam:', bufferError.message);
              throw bufferError;
            }
          }
        } catch (sendError) {
